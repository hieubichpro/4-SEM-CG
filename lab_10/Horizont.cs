...
namespace lab_10_2
{
    ...
    class Horizont
    {
        ...
        {
            if (xEdge != -1)    //Если кривая не первая
            {
        ...
        {
            if (y < TOP[x] && y > DOWN[x]) return 0;  //невидммый
            if (y >= TOP[x]) return 1;                
            return -1;                                
        ...
        public void HorizonAlgo(spacing parX, spacing parZ, Graphics painter, double tetax, double tetay, double tetaz)
        {
            ...
            for (double z = parZ.E; z >= parZ.B; z -= parZ.D)
            {
                    ...
                    for (double x = parX.B; x <= parX.E; x += parX.D)
                    {
                        ...
                        int CFlag = Visible(x_curr, y_curr);
                        ...
                        if (CFlag == PFlag)
                        {
                         ...
                         //отображаем отрезок от предыдущей до текущей.
                            if (PFlag != 0)
                            {
                        ...
                        {
                            if (PFlag == 1)  // Ищем пересечение с верхним горизонтом.
                            {
                                GetIntersection(x_prev, y_prev, x_curr, y_curr, TOP, ref xi, ref yi);
                            }
                            else             // Ищем пересечение с нижним горизонтом.
                            {
                        ...
                        {
                            if (PFlag == 0)   //Сегмент "выходит" из верхнего горизонта.
                            {
                                GetIntersection(x_prev, y_prev, x_curr, y_curr, TOP, ref xi, ref yi);
